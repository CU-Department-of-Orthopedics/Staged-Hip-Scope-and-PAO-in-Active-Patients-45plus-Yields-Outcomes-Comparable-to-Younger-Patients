...
ggsave(filename = "figure_1.tiff", path = "HiRes Figures", width = 4, height = 2, device='tiff', dpi=600, scaling = .5, units = "in")
...
names(ihot_p_sum) <- c("Group", "Month post-op", "Mean iHOT-12", "SD")

ihot_p_sum <- ihot_p_sum %>% kable(digits = 3) %>% kable_classic(html_font = 'cambria', full_width = F)

dat_ihot_long_an_no_na <- dat_ihot_long_an %>% drop_na()

## iHOT Analysis 
source(".../covariance_structure_test.R")
cov_var_fn(y = dat_ihot_long_an$ihot_score, ID = dat_ihot_long_an$mrn, time = dat_ihot_long_an$time_m, pred = dat_ihot_long_an$group, sort_by = "BIC", VC_only = F)

mod_ihot_int <- gls(ihot_score ~ group*as.factor(time_m), weights = varIdent(form = ~1), correlation = corAR1(form = ~1|mrn), data = dat_ihot_long_an, method = "REML", na.action = na.omit)    
...
emm_ihot1 <- emmeans(mod_ihot_int, ~group|as.factor(time_m), mode = "appx-satterthwaite")
...
ggsave(filename = "figure_2.tiff", path = "HiRes Figures", width = 4, height = 2, device='tiff', dpi=600, scaling = .5, units = "in")
...
names(nahs_p_sum) <- c("Group", "Month post-op", "Mean NAHS", "SD")

nahs_p_sum <- nahs_p_sum %>% kable(digits = 3) %>% kable_classic(html_font = 'cambria', full_width = F)
...
emm_nahs1 <- emmeans(mod_nahs_int, ~group|as.factor(time_m), mode = "appx-satterthwaite")
